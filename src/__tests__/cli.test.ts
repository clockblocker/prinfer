...
describe("CLI", () => {
  ...
  test("shows error for missing file", async () => {
    const { stderr, exitCode } = await runCli(["/nonexistent/file.ts", "foo"]);
  ...
  test("accepts --project option", async () => {
    const projectPath = path.join(import.meta.dir, "..", "..", "tsconfig.json");
    const { stdout, exitCode } = await runCli([sampleFile, "add", "--project", projectPath]);
    expect(stdout).toContain("number");
    expect(exitCode).toBe(0);
  });
  ...
  test("accepts -p option", async () => {
    const projectPath = path.join(import.meta.dir, "..", "..", "tsconfig.json");
    const { stdout, exitCode } = await runCli([sampleFile, "add", "-p", projectPath]);
    expect(stdout).toContain("number");
    expect(exitCode).toBe(0);
  });
});
